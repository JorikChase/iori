...
# the yellow pigment's absorption edge, nm — the ENGINE's (index.html YELLOW_EDGE_NM). 500 until engine 0.9.5: its blues were
# violet and a cyan-blue iris out of reach; 450 fits all four eyes better and leaves their camera grades ≈ identity
# (study/11 §3.3, tools/t7/yellow_edge.py). The published eyes up to 0.9.5 were fitted at 500.
YELLOW_EDGE_NM = 450.0
yel = 1 - 1 / (1 + np.exp(-(lam - YELLOW_EDGE_NM) / 14))
...
def main():
    ...
    # study/11 T7 (ref 09's blue): --yellow-edge NM overrides the engine's edge (YELLOW_EDGE_NM) for a study; outputs then
    # carry it in their names. The export always records the edge it was fitted with (`lut.yellowEdgeNm`).
    YEDGE = float(sys.argv[sys.argv.index('--yellow-edge') + 1]) if '--yellow-edge' in sys.argv else YELLOW_EDGE_NM
    global yel; yel = 1 - 1 / (1 + np.exp(-(lam - YEDGE) / 14))
    SFX = f'-y{int(YEDGE)}' if YEDGE != YELLOW_EDGE_NM else ''
    ...
    WHOLE = '--whole' in sys.argv; TAG = ('whole-' if WHOLE else 'proof-') + REF + SFX
    ...
    # study/11 T7 (ref 09's blue): the sheet's own colours join the targets — it owns most of an iris, and a blue
    # pupillary zone lives there — and the grade is judged per HUE FAMILY: a plain mean let a 9 % blue go unseen, and the
    # mildest-grade rule below then dropped it (09's blue at ΔE 10+ while the rest fitted at 1.7).
    sheet_samp = cv2.GaussianBlur(plab, (0, 0), 0.5 * CELL_MM * 1000 / UM)[sheet_px & (plab[..., 0] > 22)]
    sheet_samp = sheet_samp[np.random.RandomState(7).choice(len(sheet_samp), min(1500, len(sheet_samp)), replace=False)] if len(sheet_samp) else sheet_samp
    gt = np.concatenate([np.repeat(np.stack(list(tgt.values())), 6, 0), samp, sheet_samp.astype(np.float32)]); tried = []
    ch_ = np.hypot(gt[:, 1], gt[:, 2]); hue_ = np.where(ch_ > 4, (np.degrees(np.arctan2(gt[:, 2], gt[:, 1])) % 360 // 45).astype(int), 8)   # 8 hue sectors + neutral
    fams = [m for m in (hue_ == k for k in range(9)) if m.mean() >= 0.02]                                   # families of at least 2 %
    def balanced(dE): return float(np.mean([dE[m].mean() for m in fams]))
    for g in (1.0, 1.2, 1.4, 1.6, 1.8, 2.0, 2.4, 2.8):
        for rot in (-20, -10, 0, 10, 20, 30, 40):
            _, _, _, dE = invert(gt, LUT, g, rot); tried.append((balanced(dE), g, rot))
    ...
    _, G, ROT = best; print(f'camera grade: chroma ×{G}, hue {ROT:+d}° (ΔE {best[0]:.2f}, the mean over {len(fams)} hue families of {len(gt)} colour targets)')
