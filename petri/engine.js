...
const KIND = { inoculate: 0, nutrient: 1, toxin: 2, attract: 3, erase: 4, scratch: 5, pour: 6 };
...
export async function createEngine(canvas, opts = {}) {
  ...
  const B = { cells: [buf(cells * 16), buf(cells * 16)], sub: [buf(cells * 16), buf(cells * 16)], dep: buf(cells * 4), depId: buf(cells * 4),
    agents: buf(maxAgents * AGENT_BYTES), order: buf(maxAgents * 4), sim: buf(SIM_BYTES, GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST), view: buf(VIEW_BYTES, GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST) };
  E.buffers = B;
  ...
  const G = { cell: [0, 1].map((k) => bg(pCell, [B.sim, B.cells[k], B.cells[1 - k], B.sub[k], B.sub[1 - k], B.dep, B.depId])),
    agent: [0, 1].map((k) => bg(pAgent, [B.sim, B.agents, B.cells[k], B.sub[k], B.dep, B.depId])),
    divide: bg(pDivide, [B.sim, B.agents]),
    render: [0, 1].map((k) => bg(pRender, [B.sim, B.view, B.cells[k], B.sub[k]])) };
  ...
  function stamp(kind, cx, cy, r, amount, slot, salt) {
    ...
    pass.setPipeline(pStamp); pass.setBindGroup(0, bg(pStamp, [B.sim, U, B.cells[S.cur], B.sub[S.cur]]));
  ...
  function slotFor(orgId) {
    ...
    S.slots[s] = { org: o.id, name: o.name, kernel: o.kernel, P: [...o.P], Q: [...o.Q], ramp: [...o.ramp] };
    return s;
  }
  function spawnAgents(slot, c, rc, salt) {
    const M = Math.min(8192, Math.max(64, Math.round(Math.PI * rc * rc * 1.5)), Math.floor((maxAgents - S.agentHigh) / AGENT_STRIDE));
    if (M <= 0) return 0;
  ...
  function applyOp(op, index) {
    ...
    switch (op.tool) {
      case 'inoculate': {
        const slot = slotFor(op.organism); if (!slot) return false;
        pts.forEach((mm, k) => { const c = toCell(mm); stamp(KIND.inoculate, c[0], c[1], rc, 1, slot, salt + k); if (S.slots[slot].kernel === 1) spawnAgents(slot, c, rc, salt + k); });
        S.nInoc++; break;
      }
      case 'nutrient': case 'toxin': case 'attract': case 'erase': case 'scratch':
  ...
  E.newDish = ({ seed = S.seed, nutrient = S.dish.nutrient, agar = S.dish.agar, temp = S.dish.temp } = {}) => {
    ...
    for (const b of [B.cells[0], B.cells[1], B.sub[0], B.sub[1], B.dep, B.depId, B.agents]) enc.clearBuffer(b);
  ...
  E.step = (k = 1) => { for (let i = 0; i < k; i++) step(); };
  E.run = async (k, chunk = 50) => { for (let i = 0; i < k; i += chunk) { E.step(Math.min(chunk, k - i)); await device.queue.onSubmittedWorkDone(); } };
  ...
  let last = 0;
  E.frame = (now) => {
    if (E.playing) { const t0 = performance.now(); E.step(E.spf); E.stepMs += ((performance.now() - t0) / E.spf - E.stepMs) * 0.1; }
  ...
  E.hash = async () => {                                   // FNV-1a over the bit patterns of cells + substrate + agents
    let h = 0x811c9dc5;
    for (const ab of [await read(B.cells[S.cur], cells * 16), await read(B.sub[S.cur], cells * 16), await E.readAgents()]) { const u = new Uint32Array(ab); for (let i = 0; i < u.length; i++) { h ^= u[i]; h = Math.imul(h, 0x01000193); } }
