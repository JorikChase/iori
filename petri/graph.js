...
export function skeleton(mask, n, maxIter = 60) {
  const s = Uint8Array.from(mask);
  const at = (x, y) => (x < 0 || y < 0 || x >= n || y >= n ? 0 : s[y * n + x]);
  const doomed = [];
  for (let iter = 0; iter < maxIter; iter++) {
    ...
    for (let sub = 0; sub < 2; sub++) {
      ...
      for (let y = 1; y < n - 1; y++) for (let x = 1; x < n - 1; x++) {
        if (!s[y * n + x]) continue;
        const p = NB8.map(([dx, dy]) => at(x + dx, y + dy));
        // p indexed E, NE, N, NW, W, SW, S, SE
        const [E, NE, N, NW, W, SW, S, SE] = p;
        const C = ((!E && (NE || N)) ? 1 : 0) + ((!N && (NW || W)) ? 1 : 0)
                + ((!W && (SW || S)) ? 1 : 0) + ((!S && (SE || E)) ? 1 : 0);
        if (C !== 1) continue;
        const N1 = (NE || N ? 1 : 0) + (NW || W ? 1 : 0) + (SW || S ? 1 : 0) + (SE || E ? 1 : 0);
        const N2 = (N || NW ? 1 : 0) + (W || SW ? 1 : 0) + (S || SE ? 1 : 0) + (E || NE ? 1 : 0);
        const Nm = Math.min(N1, N2);
        if (Nm < 2 || Nm > 3) continue;
        const cond = sub === 0 ? ((NE || N || !SE) && E) : ((SW || S || !NW) && W);
        if (!cond) doomed.push(y * n + x);
      }
...
export function buildGraph(skel, dist, n, cellMm, { pruneMm = 0.25, spurWidths = 3 } = {}) {
  ...
  const keep = edges.filter((e) => {
    ...
    return tip < 0 || e.len_mm >= Math.max(pruneMm, spurWidths * e.width_mm);
  });
  const live = new Set();
