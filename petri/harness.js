...
export async function runAll({ tag = 'dev', save = true, skip = [], tiers = ['T0', 'T1', 'T4'] } = {}) {
  ...
  out.pass = ['T0', 'T1'].every((t) => Object.values(out[t] || {}).every((r) => r.pass)) && out.gpuErrors.length === 0;
...
export const T1 = {
  ...
  // Eden growth: a compact cluster whose interface width grows as t^beta, KPZ beta = 1/3. Two
  // protocol findings, both measured: it must be sampled WHILE growing (once the cluster meets the
  // wall the width saturates and the fit collapses), and the curvature coupling is what makes it
  // Eden at all - with it off, growth is uncorrelated and beta is 0.52, the random-deposition value.
  // Sweep at draft: compact 0 -> 0.518, 0.5 -> 0.259, 1.5 -> 0.355, 3 -> 0.334.
  async edenRoughness() {
    const e = E(); await clean(e, { nutrient: 1.0, agar: 0.5 });
    await grow(e, 'eden-cluster', null, null, 200, 0.4);
    const s = await growTo(e, 14, 40000, 350);
    const use = s.filter((r) => r.radius_mm > 1.5 && r.radius_mm < 15);
    const ex = exponent(use.map((r) => r.step), use.map((r) => r.width_mm));
    return { pass: ex.slope > 0.25 && ex.slope < 0.45 && ex.r2 > 0.7, beta: ex.slope, r2: ex.r2,
             target: 0.333, tol: 0.10, samples: use.length, note: 'KPZ on a lattice' };
  },
