...
export const KPARAMS = {
  ...
  3: [['eta', 'P', 0, 0, 4, 0.05], ['stick', 'P', 1, 0.01, 1, 0.01], ['absorb', 'P', 2, 0.01, 0.6, 0.01], ['compact', 'P', 3, 0, 2, 0.05]],
...
fn kLaplacian(p: vec2i, i: u32, slot: u32, c: vec4f, s: vec4f, ds: ptr<function, vec4f>) -> vec4f {
  let P = S.slotP[slot];
  if (c.r > 0.5) { (*ds).r -= P.z * s.r; return vec4f(1.0, min(c.g + 0.0015, 1.0), c.b, f32(slot)); }
  var cnt = 0.0; for (var k = 0u; k < 8u; k++) { cnt += W8[k] * select(0.0, 1.0, same(p + N8[k], slot).r > 0.5); }
  if (cnt == 0.0) { return vec4f(0.0); }
  let prob = P.y * pow(clamp(s.r, 0.0, 1.0), P.x) * (1.0 + P.w * (cnt - 1.0)) * S.temp * rs();
...
// kind: 0 inoculate, 1 nutrient, 2 toxin, 3 attractant, 4 erase, 5 scratch (sheath), 6 pour plate
...
@compute @workgroup_size(16, 16) fn main(@builtin(global_invocation_id) gid: vec3u) {
  ...
  switch T.kind {
    ...
    case 6u: { cells[i] = vec4f(0.0); sub[i] = vec4f(T.amount, 0.0, 0.0, 0.0); }   // pour a fresh plate
    default: {}
  }
}
`;
