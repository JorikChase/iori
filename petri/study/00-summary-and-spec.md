...
**Decided by iori, 2026-09-20:** the project is `petri`, in `petri/`, registered as https://iori.me/petri/. **The shell follows iris-engine's Program Manager idiom, a little more minimal** (caption, menu bar with the status at its right, tool windows that minimise to a shortcut row, Urbanist) — this supersedes layout C of chapter 06 for the chrome; chapter 06's instrument ideas (objective rungs, reticle, true-scale ghost, telemetry as readouts, clinical numbers) stay, inside that shell and in the viewport. **Organism breadth must match flame-strains' automata catalogue**: a handful of kernels, every named organism a data row. Built as ten kernels and 93 rows (`kernels.js`, `catalog.js`); this makes the multi-kernel cell pass the P1 core instead of a P5 add-on, and moves bricks / packed contract / agent sort to the P1 remainder (see `../HANDOFF.md`).
