...
win('dish', 'Dish', innerWidth - 290, 60, (body) => {
  ...
  const out = h('div', 'pt-note'); const meas = h('div', 'pt-brow'); meas.append(button('Measure', async () => { const st = await E.stats(); out.innerHTML = `agents ${st.agents.toLocaleString()}<br>` + st.slots.map((s) => `${s.name}: ${s.area_mm2} mm²`).join('<br>'); }));
...
function act(mm, path, tool = U.tool) {
  ...
  switch (tool) {
    case 'needle': case 'streak': {
      ...
      if (!ok) alert('All eight organism slots of this dish are in use.'); else { U.slot = E.sim.slots.findIndex((s) => s && s.org === o.id); if (!E.playing && E.sim.step === 0) E.playing = true; }
      break;
    }
...
E.onFrame = () => {
  ...
  status.textContent = `${fmtT(E.sim.step)}  ·  step ${E.sim.step}  ·  x ${c[0].toFixed(2)}  y ${c[1].toFixed(2)} mm  ·  r ${r.toFixed(2)}  θ ${th.toFixed(0)}°  ·  field ${fw >= 1 ? fw.toFixed(1) + ' mm' : (fw * 1000).toFixed(0) + ' µm'}  ·  ${E.frameMs.toFixed(0)} ms`;
