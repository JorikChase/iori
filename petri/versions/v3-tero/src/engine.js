...
export async function createEngine(canvas, opts = {}) {
  ...
  const B = { cells: [buf(cells * 16), buf(cells * 16)], sub: [buf(cells * 16), buf(cells * 16)], dep: buf(cells * 4), depId: buf(cells * 4),
    agents: buf(maxAgents * AGENT_BYTES), order: buf(maxAgents * 4), cond: buf(Math.max(256, n / 4) ** 2 * 4), sim: buf(SIM_BYTES, GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST), view: buf(VIEW_BYTES, GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST) };
  E.buffers = B;
  ...
  const G = { cell: [0, 1].map((k) => bg(pCell, [B.sim, B.cells[k], B.cells[1 - k], B.sub[k], B.sub[1 - k], B.dep, B.depId, B.cond])),
  ...
  E.newDish = ({ seed = S.seed, nutrient = S.dish.nutrient, agar = S.dish.agar, temp = S.dish.temp } = {}) => {
    ...
    for (const b of [B.cells[0], B.cells[1], B.sub[0], B.sub[1], B.dep, B.depId, B.agents, B.cond]) enc.clearBuffer(b);
  ...
  E.updateGraph = () => {
    ...
    E.graph.busy = (async () => {
      ...
      const foodMm = S.ops.filter((o) => o.tool === 'nutrient' && (o.amount ?? 1) > S.dish.nutrient)
