...
fn kPhysarum(p: vec2i, slot: u32, c: vec4f, d: u32, s: vec4f, ds: ptr<function, vec4f>) -> vec4f {
  ...
  // P2 feedback: a vein that carries flux (Tero conductivity from the graph worker) keeps its trail;
  // one that stopped carrying flux falls back to the ordinary persistence and fades.
  var keep = Q.y;
  if (isAdaptive(slot)) { keep = mix(Q.y, 0.985, clamp(condAt(p), 0.0, 1.0)); }
  var T = sum / 9.0 * keep;
  let fd = f32(d);
