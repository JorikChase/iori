...
// ---------------------------------------------------------------------------------------------- frame
document.body.append(h('div', '', '<div id="pt-cap"><button id="pt-ctl" class="pt-ctl" aria-label="Site menu" title="iori.me · site menu"></button><span class="pt-capt0">Petri</span></div>'),
  Object.assign(h('div'), { id: 'pt-menubar' }), Object.assign(h('div'), { id: 'pt-icons' }), Object.assign(h('div'), { id: 'pt-scale' }));
...
// ---------------------------------------------------------------------------------------------- site menu
// The site burger lives in the frame, as in the iris engine's shell (iris-engine/ui31.js): the caption's
// control box shows the 3DIE favicon as a 16-colour bitmap at exactly the box's pixel size, and opens the
// site's links. The links are read from the generated #site-menu markup, so `site.py menu` stays the one
// source; the floating burger itself is hidden (ui.css).
...
function quantise(c, n) {
  const im = c.getImageData(0, 0, n, n), d = im.data;
  for (let i = 0; i < n * n; i++) {
    const o = i * 4; if (d[o + 3] < 128) { d[o + 3] = 0; continue; }
    const t = (B4[(Math.floor(i / n) & 3) * 4 + ((i % n) & 3)] / 16 - 0.47) * 56; let best = 0, bd = 1e9;
...
let siteImg = null;
function siteIcon() {
  const b = $('pt-ctl'); if (!b) return;
  const px = Math.max(8, b.clientHeight || 18);
  const paint = () => {
    const a = document.createElement('canvas'); a.width = a.height = 64; const ac = a.getContext('2d'); ac.imageSmoothingQuality = 'high'; ac.drawImage(siteImg, 0, 0, 64, 64);
    ...
    const im = c.getImageData(0, 0, px, px), d = im.data;      // the logo's muted colours and hatched ground fall between VGA entries
    for (let o = 0; o < d.length; o += 4) {
      const l = 0.3 * d[o] + 0.59 * d[o + 1] + 0.11 * d[o + 2], sat = Math.max(d[o], d[o + 1], d[o + 2]) - Math.min(d[o], d[o + 1], d[o + 2]);
      if (sat < 38) { if (l < 88) d[o] = d[o + 1] = d[o + 2] = 0; else { d[o] = 40; d[o + 1] = 170; d[o + 2] = 140; } }   // the large grey-green triangle gets the palette's teal
      else for (let k = 0; k < 3; k++) d[o + k] = Math.max(0, Math.min(255, l + (d[o + k] - l) * 2.6 + 18));
    }
    c.putImageData(im, 0, 0); quantise(c, px);
    b.style.backgroundImage = `url(${cv.toDataURL()})`; b.classList.add('pt-site');
  };
  if (siteImg) return paint();
  const im = new Image(); im.onload = () => { siteImg = im; paint(); }; im.src = '../icon/ios/180.png';
}
function siteItems() {
  const out = [];
  document.querySelectorAll('#site-menu-panel > li').forEach((li) => {
    const a = li.querySelector('a');
    if (!a) { out.push('-', { label: li.textContent.trim(), head: true }); return; }
    const sm = a.querySelector('small'), name = sm ? a.textContent.replace(sm.textContent, '').trim() : a.textContent.trim();
    out.push({ label: (sm ? sm.textContent.trim() + ':&nbsp; ' : '') + name, chk: a.getAttribute('aria-current') === 'page', run: () => { location.href = a.href; } });
  });
  return out;
}
$('pt-ctl').addEventListener('pointerdown', (e) => {
  e.stopPropagation();
  const top = $('pt-ctl'), was = openMenu && openMenu.top === top; closeMenu(); if (was) return;
  const el = h('div', 'pt-menu'), r = top.getBoundingClientRect(); el.style.left = r.left + 'px'; el.style.top = r.bottom + 'px';
  for (const it of siteItems()) {
    if (it === '-') { el.append(h('div', 'pt-sep')); continue; }
    const mi = h('div', 'pt-mi' + (it.chk ? ' chk' : '') + (it.head ? ' head' : ''), it.label);
    if (it.run) mi.addEventListener('click', () => { closeMenu(); it.run(); });
    el.append(mi);
  }
  document.body.append(el); top.classList.add('open'); openMenu = { el, top };
});
...
function act(mm, path, tool = U.tool) {
  ...
  switch (tool) {
    ...
    case 'food': E.op({ tool: 'flake', at, r, amount: 3 }); break;          // a solid flake, not dissolved nutrient
...
addEventListener('resize', () => { WINS.forEach((W) => W.place()); siteIcon(); });
