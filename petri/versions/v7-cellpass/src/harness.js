...
export const T0 = {
  ...
  // P2: replay stays bit-exact through the asynchronous graph worker, because the stepping waits for
  // each update at a fixed step and the update depends only on the state at that step
  async replayAdaptive() {
    const e = E(), hashes = [];
    const id = { petri: 1, seed: 29, dish: { nutrient: 0.35, agar: 0.5, temp: 1 }, ops: [
      { t: 0, tool: 'inoculate', organism: 'physarum-polycephalum-adaptive-network', at: [0, 0], r: 2.5 },
      { t: 0, tool: 'flake', at: [12, 4], r: 1.4, amount: 3 }, { t: 0, tool: 'flake', at: [-10, -8], r: 1.4, amount: 3 }] };
    for (let k = 0; k < 2; k++) { e.importID(id); await e.run(1000, 250); hashes.push(await e.hash()); }
    const updates = e.graph.history.length;
    return { pass: hashes[0] === hashes[1] && updates >= 3, hashes, graphUpdates: updates };
  },
...
export const T4 = {
  ...
  // Same scene, each kernel in its own timed pass (E.profile) — says WHICH pass a cost change is in.
  async kernelCost() {
    const e = E(); e.importID(SCENE(19)); await e.run(200);
    const pr = await e.profile(128);
    return pr ? { ...pr, total: +(pr.agent + pr.divide + pr.cell).toFixed(3), n: e.n } : { skipped: 'no timestamp-query' };
  },
};
...
// what a vein is.
import { extract, extractValues, networkStats, t2Gates, largestComponent } from './graph.js';
import { teroMetrics, fixture36 } from './fixture.js';

export const T2 = {
  // Tero et al. 2010 in food-source mode: 36 food sources, the organism starts at one, the metrics are
  // read off the tubes the adaptation keeps conducting. Tero's own geometry (cities round Tokyo) was not
  // re-digitised, so the layout is a fixed synthetic one at his source count — compare by regime:
  // Physarum 1.75 ± 0.30, 0.85 ± 0.04, 0.86 ± 0.04; Tokyo rail ≈ 1.8, 0.85, 0.96. It runs its own
  // draft engine because covering the fixture takes tens of thousands of steps.
  async teroFixture({ steps = 36000, seeds = [2010, 2011, 2012] } = {}) {
    // Replicated like Tero's own numbers (n = 21, mean ± SD across organisms): the same 36-source layout
    // for every run, a different organism (seed) each time. Rule fixed BEFORE the replicated runs were
    // made: every seed must reach >= 80 % coverage, and the MEAN of each metric across seeds must sit
    // in the bands, which are unchanged. (A single run's MD/MST moved 0.72-0.95 between samples.)
    const { createEngine } = await import('./engine.js');
    const food = fixture36();
    const runs = [];
    for (const seed of seeds) {
      const e2 = await createEngine(new OffscreenCanvas(64, 64), { quality: 'draft', seed, nutrient: 0.3, agar: 0.5 });
      e2.importID({ petri: 1, seed, dish: { nutrient: 0.3, agar: 0.5, temp: 1 }, ops: [
        // Tero's conditions: a plasmodium large enough for the task, and oat flakes that stay food for the
        // whole run. They are SOLID flakes (tool 'flake', a persistent source): a blob of dissolved
        // nutrient spread into the agar within ~1500 steps and fell under the attractant threshold by
        // ~10 000, which is what made the network decay (diag/fixture-trace.js).
      ...
      for (let done = 0; done < steps; done += 3000) {
        await e2.run(3000, 300);
        if (e2.graph.net) {
          const span = e2.n * e2.cellMm, foodGrid = food.map(([x, y]) => [x + span / 2, y + span / 2]);
          trace.push({ step: e2.sim.step, ...teroMetrics(e2.graph.net, foodGrid) });
        }
      }
      e2.device.destroy();
      runs.push({ seed, final: trace[trace.length - 1] || {}, peak: trace.reduce((b, m) => (!b || (m.coverage || 0) > (b.coverage || 0) ? m : b), null), trace });
    }
    const mean = (k) => runs.reduce((s, r) => s + (r.final[k] || 0), 0) / runs.length;
    const sd = (k) => { const m = mean(k); return Math.sqrt(runs.reduce((s, r) => s + ((r.final[k] || 0) - m) ** 2, 0) / Math.max(runs.length - 1, 1)); };
    const summary = { n: runs.length, coverageMin: Math.min(...runs.map((r) => r.final.coverage || 0)),
      TL_MST: [+mean('TL_MST').toFixed(3), +sd('TL_MST').toFixed(3)], MD_MST: [+mean('MD_MST').toFixed(3), +sd('MD_MST').toFixed(3)],
      FT: [+mean('FT').toFixed(3), +sd('FT').toFixed(3)] };
    // Bands as first written, before any run: not widened to fit the data.
    const pass = summary.coverageMin >= 0.8 && summary.TL_MST[0] > 1.45 && summary.TL_MST[0] < 2.05
      && summary.MD_MST[0] > 0.75 && summary.MD_MST[0] < 1.0 && summary.FT[0] > 0.5;
    return { pass, summary, runs: runs.map(({ seed, final, peak }) => ({ seed, final, peak })),
             tero: 'Physarum n=21: TL_MST 1.75 +/- 0.30, MD_MST 0.85 +/- 0.04, FT 0.86 +/- 0.04; Tokyo rail 1.8, 0.85, 0.96',
             target: 'every seed coverage >= 0.8; mean TL_MST 1.45-2.05, MD_MST 0.75-1.0, FT > 0.5' };
  },
  ...
  async physarumNetwork() {
    ...
    e.importID({ petri: 1, seed: 31, dish: { nutrient: 0.3, agar: 0.5, temp: 1 }, ops: [
      ...
      ...[0, 1, 2, 3, 4, 5].map((k) => ({ t: 0, tool: 'flake', r: 1.4, amount: 3,
        at: [Math.cos(k * 1.047) * 22, Math.sin(k * 1.047) * 22] })),
    ] });
    await e.run(4000, 250);
    const t0 = performance.now();
    // The ENGINE's extraction (graphworker: 1 % of p99, closing), as this tier's header promises — the
    // row used its own (5 %, no closing) until 2026-09-22 and cut the forager's trail into ~100 pieces.
    // Switched on that principle before the numbers were read; the TL/MST gate still misses (1.44).
    const c = await e.readCells(), n = e.n, vals = new Float32Array(n * n);
    for (let k = 0; k < n * n; k++) if ((c[k * 4 + 3] | 0) === 1) vals[k] = c[k * 4];
    const g = largestComponent(extractValues(vals, n, e.cellMm, { ds: n >= 2048 ? 2 : 1, level: 0.01, closeR: 1, pruneMm: 0.4 }));
    ...
    return { pass: gates.passed >= 3, gatesPassed: gates.passed, gatesTotal: gates.total,
             extract_ms: +ms.toFixed(0), nodes: g.nodes.length, stats: st, gates: gates.rows };
  },
};

// Rows taken off the scorecard, kept runnable (`RETIRED.<name>()`) so their history stays reproducible.
export const RETIRED = {
  // Retired 2026-09-22 (iori agreed): with solid flakes the organism reaches for every flake at once,
  // and the whole-trail largest-component share reads that as fragmentation (ratio 1.86-1.9, gates
  // 3/4 since v5). What it tried to score — adaptation makes ONE network joining the food — is scored
  // directly by T2.teroFixture's per-organism coverage (>= 80 % of 36 sources in one conducting network).
  // P2's claim, scored: with Tero adaptation on its graph the organism consolidates into one network.
  // Same dish, same seed, forager against adaptive; the largest connected component must hold a much
  // larger share of the network, and the published gates must still hold for the adaptive one.
  async adaptationConsolidates() {
    const { extractValues, largestComponent } = await import('./graph.js');
    const e = E(); e.playing = false;
    const run = async (org) => {
      e.importID({ petri: 1, seed: 31, dish: { nutrient: 0.3, agar: 0.5, temp: 1 }, ops: [
        { t: 0, tool: 'inoculate', organism: org, at: [0, 0], r: 2.5 },
        ...[0, 1, 2, 3, 4, 5].map((k) => ({ t: 0, tool: 'flake', r: 1.4, amount: 3, at: [Math.cos(k * 1.047) * 22, Math.sin(k * 1.047) * 22] }))] });
      await e.run(6000, 300);
      const c = await e.readCells(), n = e.n, vals = new Float32Array(n * n);
      for (let k = 0; k < n * n; k++) if ((c[k * 4 + 3] | 0) === 1) vals[k] = c[k * 4];
      const g = extractValues(vals, n, e.cellMm, { ds: n >= 2048 ? 2 : 1, level: 0.01, closeR: 1, pruneMm: 0.4 });
      const L = largestComponent(g);
      return { share: L.nodes.length / Math.max(g.nodes.length, 1), nodes: g.nodes.length, bigNodes: L.nodes.length, stats: networkStats(L) };
    };
    const a = await run('physarum-polycephalum-forager'), b = await run('physarum-polycephalum-adaptive-network');
    const gates = t2Gates(b.stats);
    return { pass: b.share >= 1.5 * a.share && gates.passed === gates.total,
             shareForager: +a.share.toFixed(3), shareAdaptive: +b.share.toFixed(3), ratio: +(b.share / a.share).toFixed(2),
             gatesAdaptive: `${gates.passed}/${gates.total}`, graphUpdates: e.graph.history.length,
             lastUpdate: e.graph.stats };
  },
};
