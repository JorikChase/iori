...
@compute @workgroup_size(16, 16) fn main(@builtin(global_invocation_id) gid: vec3u) {
  ...
  // Deposits are sparse (agents touch a few % of cells): read, and clear only what was written. Nothing
  // else touches dep[i] during this pass, so this equals the unconditional exchange, without a
  // read-modify-write on every cell of the grid.
  var d = atomicLoad(&dep[i]); var did = 0u;
  if (d != 0u) { atomicStore(&dep[i], 0u); did = atomicLoad(&depId[i]); atomicStore(&depId[i], 0u); }
  // Outside the dish both ping-pong buffers stay zero: newDish clears them and no pass writes there
  // (stamp and agent passes are dish-bounded), so ~21 % of the grid needs no write at all.
  if (!inDish(p)) { return; }
