...
export function pumpElastic(g, food, phi, {
  ...
  // uptake: fluid drunk at food per unit time (fraction of the node's rest volume), removed again
  // everywhere in proportion to volume (growth / evaporation) so the total stays bounded. The drunk
  // fluid is UNLABELLED; `tracer` follows the resident sol, so its drift is transport, not the trivial
  // shift of the centre of volume that adding fluid at food would produce by itself.
  ...
  const M = Float64Array.from(V), dM = new Float64Array(Vn);           // tracer mass, starts uniform (c = 1)
  const tx = () => { let m = 0, s = 0; for (let v = 0; v < Vn; v++) { m += M[v]; s += M[v] * g.nodes[v].x; } return s / m; };
  const t0 = tx();
  let intake = 0; for (let v = 0; v < Vn; v++) intake += uptake * (food[v] || 0) * Vbar[v];
  for (let step = 0; step < periods * stepsPerPeriod; step++) {
    ...
    for (const e of g.edges) {
      ...
      // upwind tracer, Jacobi (all edges read the old concentrations — see FINDINGS §4's trap)
      const up = q > 0 ? e.a : e.b, flux = Math.abs(q) * dt * Vbar[up] * (M[up] / V[up]);
      if (q > 0) { dM[e.a] -= flux; dM[e.b] += flux; } else { dM[e.b] -= flux; dM[e.a] += flux; }
    }
    let vol = 0; for (let v = 0; v < Vn; v++) vol += V[v];
    for (let v = 0; v < Vn; v++) {
      const src = uptake * (food[v] || 0) * Vbar[v] * dt, sink = intake * dt * V[v] / vol;
      const cv = M[v] / V[v];
      V[v] = Math.max(1e-9, V[v] + dV[v] * Vbar[v] + src - sink);
      M[v] = Math.max(0, M[v] + dM[v] - cv * sink);
    }
  }
  return { drift: cx() - x0, tracerDrift: tx() - t0, order: (() => { let re = 0, im = 0; for (const f of phi) { re += Math.cos(f); im += Math.sin(f); } return Math.hypot(re, im) / Vn; })() };
}
