...
pull_from_git() {
    ...
    # Forced to HTTP/1.1. With this host's libcurl (8.5.0, Ubuntu 24.04) the
    # HTTP/2 exchange with github.com breaks after the ref advertisement: the
    # GET info/refs returns 200 and parses fine, but the follow-up request
    # fails and git drops into its dumb-HTTP fallback, which ends in a 401
    # ("could not read Username"). The repo is public, so this is not a
    # credentials problem. Pinning the git protocol version (the earlier fix)
    # does not help; forcing HTTP/1.1 does, with either protocol version.
    # Without this the deploy fetches nothing and silently ships the previous
    # commit.
...
sync_website_files() {
    ...
    rsync -a --delete --delete-excluded \
        ...
        --exclude '*.sh' \
        --exclude '*.caddy' \
        --exclude 'pages.meta.json' \
        --exclude 'parsed_data.json' \
        "$SOURCE_DIR/" "$WEB_ROOT/"
    echo "File sync complete (excluding .git, zausi, splats, heavy assets, repo tooling and notes)."
...
iori.me {
    ...
    @dotfiles path /.git* /.env* /.DS_Store /.readme /.gitattributes /.claude/* /*.md /*.py /*.sh /*.caddy /pages.meta.json /parsed_data.json /api/* /dash/* /tools/*
    handle @dotfiles {
        respond 404
    }

    # Compress text responses (HTML/JSON/SVG/XML); images and media are skipped by default
    encode zstd gzip

    # The landing page is canonical at the domain root, never at its filename
    redir /iori_INDEX.html / permanent
